...
Estado: completa (falta verificación en vivo de las llamadas reales a Gemini, ver Pendiente)

Hecho:
- Dos decisiones de arquitectura confirmadas contigo antes de codificar:
  1. **Origen de los embeddings: API de Gemini** (`gemini-embedding-001`), no un modelo local (`sentence-transformers`). Reutiliza `GEMINI_API_KEY` ya existente desde la Fase 7 — no añade proveedor/cuenta nueva — y evita meter PyTorch (~1-2GB) a un proyecto que hasta la Fase 11 solo tenía dependencias ligeras. `output_dimensionality=768` (el modelo por defecto da 3072; 768 es de sobra para unos cientos de especies y ocupa menos).
  2. **Base vectorial: LanceDB**, no Chroma (ambas avaladas por igual en CLAUDE.md/roadmap para un proyecto SQLite). Embebida, sin servidor, ficheros bajo `data/lancedb/` (mismo patrón que `data/raw/`/`data/logs/`); a diferencia de Chroma no arrastra un pipeline de embeddings ONNX por defecto que aquí no se usa (los embeddings vienen de Gemini). Verificado a mano que `.search(vector).where(sql)` hace exactamente el "filtro estructurado → similitud" del roadmap antes de escribir `store.py`.
- **Se detectó y se cerró un hueco de datos, no solo de código, antes de construir los documentos**: el roadmap pedía "habilidades" por Pokémon en el documento, pero `PokemonSpecies` nunca guardó la relación especie→habilidad (hueco anotado desde la Fase 5). En vez de omitirlo o inventar, se backfilleó desde el propio cache ya descargado de PokeAPI en la Fase 1 (`d["abilities"]` venía en cada respuesta `/pokemon/{id}`, cacheada en `data/raw/pokeapi/`, nunca se había extraído a una tabla) — tabla nueva `PokemonAbility(pokemon_species_id, ability_id, is_hidden)`, poblada en `seed_species()` (`src/db/seed.py`) sin re-scrapear nada. Confirmado en vivo: 2938 filas para 1351 especies.
- **Se omitieron a propósito dos campos que el roadmap sí pedía en el documento**: "rol típico" (sería una etiqueta inventada por este código — CLAUDE.md prohíbe justo eso) e "ítems comunes" (depende de `common_sets`, que la Fase 4 nunca pudo poblar porque ChampionsMeta no expone ese desglose). Documentado en el docstring de `src/semantic/documents.py`, no es un olvido.
- `src/semantic/documents.py`: `build_document()`, función pura (nombre, tipos, stats base, habilidades con efecto y flag de oculta, movimientos legales esta regulation con tipo/categoría/poder, % de uso con `verified` si existe). 7 tests offline, deterministas.
- `src/semantic/embeddings.py`: `embed_documents()`/`embed_query()` sobre `client.models.embed_content()` (`google-genai`, ya en el stack desde la Fase 7) — `task_type` distinto para documento (`RETRIEVAL_DOCUMENT`) vs consulta (`RETRIEVAL_QUERY`), como recomienda la documentación del modelo para embeddings asimétricos de recuperación.
- `src/semantic/store.py`: `rebuild_table()`/`search()` sobre LanceDB. `search(type_filter=...)` valida el nombre de tipo contra una regex (`^[a-z]+$`) antes de interpolarlo en el `WHERE` SQL — no hay parámetros bindeados en la API de LanceDB para esto, así que la validación explícita sustituye a la sanitización automática.
- `src/semantic/search.py`: `semantic_search_pokemon(query, type_filter?, limit?)`, la función que expone la tool.
- `src/db/seed_embeddings.py` (`python -m src.db.seed_embeddings`): reconstruye el índice completo cada vez (no incremental — el roster son unos cientos de especies como mucho, no compensa trackear qué cambió) para todas las especies legales de la regulation activa. Enganchado a `run_pipeline.py` (Fase 8) como cuarto paso, tal como pedía el roadmap ("regenerar... cada vez que cambie el roster").
- Endpoint `GET /pokemon/semantic-search?query=&type=&limit=` y tool `semantic_search_pokemon`, mismo patrón in-process de siempre. `docs/behavior_prompt.md` actualizado (tool nueva en la tabla, nota de que "rol típico" tampoco existe como etiqueta y no hay que inventarla).
- 15 tests nuevos además de los 7 de documentos: 6 de `store.py` contra LanceDB real (biblioteca embebida, no necesita red/API key) con vectores sintéticos, 2 de `search.py` con `embed_query` mockeado (para no depender de `GEMINI_API_KEY` en la suite por defecto), 3 en `tests/test_embeddings_live.py` marcados `@pytest.mark.live` (llamada real a Gemini) — el marcador `live` de la Fase 10 se amplió en su descripción para cubrir tanto scrapers como APIs de LLM con clave, ya que el mecanismo (excluir de la ejecución por defecto) es el mismo.
- **Verificación real en esta sesión**: se re-sembraron Fases 1-4 con datos reales (no había BD en este checkout, ver sesión de Fase 10) — 1351 especies + 2938 filas especie-habilidad (Fase 1), M-B con 308/310 roster legal (Fase 2, mismos números que sesiones previas), movepool completo 207/207 especies (Fase 3, ídem). **Fase 4 (uso) volvió a fallar** — ChampionsMeta sigue sin datos de M-B en `/meta`, mismo hueco que ya documentó la Fase 10, sin corrupción de datos (falla antes de escribir nada). `seed_embeddings.py` se ejecutó de verdad contra la BD real: construyó correctamente los 308 documentos (verificados a mano para Garchomp/Incineroar/Charizard — stats, habilidades incluida la oculta, y movepool legal correctos) y falló exactamente donde debía, en la llamada a Gemini, por no haber `GEMINI_API_KEY` en este entorno — ningún otro punto de fallo, confirma que toda la tubería SQL→documento funciona contra datos reales.
- 114/117 tests que tocan la BD real en verde (repetido: 3 fallan, los tres relacionados con `usage_stats` vacío por el fallo de ChampionsMeta de arriba, no bugs de esta sesión).

Pendiente:
- **Verificación manual real de las llamadas a Gemini** (`pytest -m live -k embeddings` o `python -m src.db.seed_embeddings` con `GEMINI_API_KEY` real en `.env`) — mismo tipo de pendiente que dejó la Fase 7 con el chat loop. Todo lo que no depende de la red/clave está verificado.
- Re-sembrar `usage_stats`/`notable_teams` (Fase 4) y volver a correr `seed_embeddings` cuando ChampionsMeta se recupere, para que los documentos indexados incluyan % de uso real (ahora mismo todos tienen `usage=None`).
- Nada bloqueante para pasar a Fase 13.

Decisiones tomadas (y por qué): ver los dos puntos confirmados contigo arriba (Gemini vs local, LanceDB vs Chroma).

---

## Próxima sesión
Añadir `GEMINI_API_KEY` real a `.env` y correr `python -m src.db.seed_embeddings` (o `pytest -m live`) para cerrar la verificación manual de la Fase 12. Re-sembrar `usage_stats` cuando ChampionsMeta salga de su estado degradado (Fase 10/4). Después, empezar Fase 13 — scraping mensual de percepción comunitaria (foros/Reddit/YouTube, resumen vía Claude, `community_meta_snapshots`, tool `get_community_buzz`).
